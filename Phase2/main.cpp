...
// ===========================================================================
// Helper: Process Individual Queries
// ===========================================================================
json process_query(const Graph& graph, const json& query) {
    ...
    // -------------------------------------------------------
    // 1. K Shortest Paths (Exact - Yen's Algorithm) [cite: 210]
    // -------------------------------------------------------
    if (type == "k_shortest_paths") {
        ...
        // Instantiate Yen's algorithm wrapper
        Yen yenAlgo(&graph);
    ...
    // -------------------------------------------------------
    // 2. K Shortest Paths (Heuristic) [cite: 213]
    // -------------------------------------------------------
    ...
    // -------------------------------------------------------
    // 3. Approximate Shortest Path (Batch) [cite: 219]
    // -------------------------------------------------------
    else if (type == "approx_shortest_path") {
        // This query type usually contains a batch of sub-queries
...
int main(int argc, char* argv[]) {
    ...
    // Support both "events" (from sample) and raw list (implied by some contexts)
    ...
    if (events.is_array()) {
        for (const auto& query : events) {
            ...
            json result;
            try {
                //  Wrap query processing in try-catch
            ...
            // [cite: 282] Append processing time
            if (!result.empty()) {
